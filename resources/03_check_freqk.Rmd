...
```{r}
neutfreqs <- list.files("/global/scratch/users/milesroberts/moi_lab_projects/grenenet-phase2/workflow/slim_results", pattern = "*freqs.tsv", full.names = T)

no_col <- lapply(neutfreqs[1:5], FUN = function(x) { max(count.fields(x, sep = "\t")) })

neutfreqs <- mapply(function(x,y){ read.table(x, fill = T, col.names = 1:y)}, neutfreqs[1:5], no_col)

tbl <-
    list.files("/global/scratch/users/milesroberts/moi_lab_projects/grenenet-phase2/workflow/slim_results", pattern = "*freqs.tsv", full.names = T) %>% 
  max(count.fields(., sep = "\t"))
    #map_df(~read_table(.))
```
