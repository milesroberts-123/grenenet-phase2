...
## buffalo gt curves
gt_curves <- function(covmat, calcabs = TRUE) {
  ...
  for(j in 2:nrow(covmat)){
    var_sub <- variances[1:j]
    cov_sub <- covariances[1:((j-1)*(j))/2]
    
    if (calcabs == TRUE) {
	    sum_cov <- 2*sum(abs(cov_sub), na.rm = T)
    } else {
	    sum_cov <- 2*sum(cov_sub, na.rm = T)
    }
    
    gts = c(gts, sum_cov/(sum(var_sub) + sum_cov))
  }
...
for(i in 1:ncol(dp0)){
  ...
  for(j in 2:nrow(tmat)){
    ...
    gts = c(gts, sum(2*abs(covariances), na.rm = T)/(sum(variances) + sum(2*abs(covariances), na.rm = T)))
  }
...
#no_col <- lapply(neutfreqs[1], FUN = function(x) { max(count.fields(x, sep = "\t")) })

neutfreq_files <- sample(neutfreq_files, size = 540, replace = F)
...
# Source - https://stackoverflow.com/a/5890979
# Posted by Chase
# Retrieved 2026-02-21, License - CC BY-SA 3.0
#reshape(neutfreqs[,c("X1", "pos", "dp")], idvar = "pos", timevar = "X1", direction = "wide")

#neutfreqs$gen_pos <- paste(neutfreqs$X1, neutfreqs$pos, )
#spread(neutfreqs[,c("X1", "pos", "dp")], key = "pos", value = "dp")
...
# Retrieved 2026-02-21, License - CC BY-SA 3.0
  ...
  dpmat <- dpmat[complete.cases(dpmat),-1:-6]

  dpcov_mat <- cov(dpmat[,-1])

  sim_curve <- gt_curves(dpcov_mat, calcabs = TRUE)
...
# average across reps in a site
mean_sims = sim_curves %>% group_by(gen, sigma, alpha, adjust) %>% summarize(gts = mean(gts))

ggplot(subset(mean_sims, gen <= 5), aes(x = gen, y = gts, group = interaction(sigma, alpha), color = as.factor(sigma), shape = as.factor(alpha))) +
  geom_point() +
  scale_color_scico_d() +
  facet_wrap(~adjust, labeller = as_labeller(c("TRUE" = "Ne constant", "FALSE" = "Ne lowered"))) +
  geom_line() +
  theme(legend.position = "top") +
  #coord_fixed() +
  #coord_equal(ratio = 5/0.75) +
  labs(x = "Generation", y = "G(t)", color = "Self\nrate", shape = "|s|")

ggsave(paste("../results/", today, "/simulated_linked_selection.pdf", sep = ""), height = 4.5, width = 7, device = cairo_pdf)

# dpcov_long <- melt(dpcov_mat)
# 
# ggplot(subset(dpcov_long, Var1 == 10251 & Var2 > 10251), aes(x = Var2, y = abs(value))) +
#   geom_smooth() +
#   geom_point()
```
